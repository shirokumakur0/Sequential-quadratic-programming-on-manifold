...
dim_set = [10, 15, 20, 25, 30];  %dimension of the Adjacency Matrix
...
for repeat = 1 : n_repeat
    ...
    for dim = dim_set
        ...
        for density = density_set
            ...
            result = clientconstraint_modified_artificial_oblique_with_SQP(L, rank, options, specifier, setting);
            result = result(:);
            param = [dim; density; repeat];
            outputdata = [result; param]';
            
            filename = sprintf('with_SQP_zz_AO_Dim%d.dat', dim);
