        ...
        for cratio = col_ratio
            for rratio = fixedrank_ratio
                for dimratio = lc_dim_ratio
                    ...
                    result = clientconstraint_rank_constraints_nnqflc_with_SQP(rdim, cdim, rank, lc_dim, P, PA, options, specifier, setting);
                    result = result(:);
                    param = [rdim; cdim; rank];
                    outputdata = [result; param]';

                    filename = sprintf('with_SQP_zz_RC_nnqflc_RDim%dCDim%d.dat', rdim,cdim);
