 function data = clientconstraint_simplified_stiefel_Kmeans_with_SQP(D, rankY, methodoptions, specifier, setting)
...
%     ------------------------- Solving ---------------------------
    ...
    if specifier.ind(1)
        ...
        filename = sprintf('SKNN_Mini-Sum-Max_nrep%dNum%dDim%dRank%d.csv',setting.repeat,...
    ...
    if specifier.ind(2)
        ...
        filename = sprintf('SKNN_ALM_nrep%dNum%dDim%dRank%d.csv',setting.repeat,...
            setting.data_num, setting.data_dim, setting.rank);
        struct2csv(info, filename);
    ...
    if specifier.ind(3)
        ...
        filename = sprintf('SKNN_LQH_nrep%dNum%dDim%dRank%d.csv',setting.repeat,...
            setting.data_num, setting.data_dim, setting.rank);
        struct2csv(info, filename);
    ...
    if specifier.ind(4)
        ...
        filename = sprintf('SKNN_LSE_nrep%dNum%dDim%dRank%d.csv',setting.repeat,...
            setting.data_num, setting.data_dim, setting.rank);
        struct2csv(info, filename);
    ...
    if specifier.ind(5)
        ...
        [xfinal, fval, ~, output] = fmincon(@(v) costFunfmincon(v), x0(:), [], [], [], [], zeros(N*rankY, 1), [], [], options);
        time = toc(timetic);
        
        history.iter(1,:) = [];
        history.cost(1,:)  = [];
        
        filename = sprintf('SKNN_fmincon_interior_point_nrep%dNum%dDim%dRank%d.csv',setting.repeat,...
            setting.data_num, setting.data_dim, setting.rank);
    ...
    if specifier.ind(6)
        ...
        filename = sprintf('SKNN_fmincon_SQP_nrep%dNum%dDim%dRank%d.csv',setting.repeat,...
            setting.data_num, setting.data_dim, setting.rank);
        struct2csv(history, filename);        
    ...
    if specifier.ind(7)
        ...
        filename = sprintf('SKNN_Riemannian_SQP_nrep%dNum%dDim%dRank%d.csv',setting.repeat,...
            setting.data_num, setting.data_dim, setting.rank);
        struct2csv(info, filename);
