...
%% Calculating by solvers
    ...
    if specifier.ind(2)
        ...
        time = toc(timetic);
        filename = sprintf('RC_nnlc_ALM_nrep%dRowdim%dColdim%dRank%dLcdim%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank, setting.lc_dim);
    ...
    if specifier.ind(3)
        ...
        time = toc(timetic);
        filename = sprintf('RC_nnlc_LQH_nrep%dRowdim%dColdim%dRank%dLcdim%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank, setting.lc_dim);
        struct2csv(info, filename);
        [maxviolation, meanviolation, cost] = evaluation(problem, xfinal, condet);
        % maxviolation = max(maxviolation, manifoldViolation(xfinal));
    ...
    if specifier.ind(4)
        ...
        time = toc(timetic);
        filename = sprintf('RC_nnlc_LSE_nrep%dRowdim%dColdim%dRank%dLcdim%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank, setting.lc_dim);
        struct2csv(info, filename);        
        [maxviolation, meanviolation, cost] = evaluation(problem, xfinal, condet);
        % maxviolation = max(maxviolation, manifoldViolation(xfinal));
 ...
 %   end
    ...
    if specifier.ind(7)
        ...
        time = toc(timetic);
        filename = sprintf('RC_nnlc_Riemannian_SQP_nrep%dRowdim%dColdim%dRank%dLcdim%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank, setting.lc_dim);
        struct2csv(info, filename);        
        [maxviolation, meanviolation, cost] = evaluation(problem, xfinal, condet);
        % maxviolation = max(maxviolation, manifoldViolation(xfinal));
    ...
    filename = sprintf('RC_nnlc_Info_nrep%dRowdim%dColdim%dRank%dLcdim%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank, setting.lc_dim);
