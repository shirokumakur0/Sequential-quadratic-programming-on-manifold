...
%    end
%end
%zeroeig_num = randsample(m*n, deg_num);  % random sampling for indices of eigenvalues to be replaced with zero
%for ii = zeroeig_num
%   T(ii,ii) = 0; 
%end
%C = U * T * U';
...
%lc_constraints_cost = cell(m * n, 1);
%lc_constraints_grad = cell(m * n, 1);
%lc_constraints_hess = cell(m * n, 1);
...
%% Calculating by solvers
    ...
    if specifier.ind(2)
        ...
        time = toc(timetic);
        filename = sprintf('RC_nn_ALM_nrep%dRowdim%dColdim%dRank%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank);
    ...
    if specifier.ind(3)
        ...
        time = toc(timetic);
        filename = sprintf('RC_nn_LQH_nrep%dRowdim%dColdim%dRank%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank);
        struct2csv(info, filename);
        [maxviolation, meanviolation, cost] = evaluation(problem, xfinal, condet);
        % maxviolation = max(maxviolation, manifoldViolation(xfinal));
    ...
    if specifier.ind(4)
        ...
        time = toc(timetic);
        filename = sprintf('RC_nn_LSE_nrep%dRowdim%dColdim%dRank%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank);
        struct2csv(info, filename);        
        [maxviolation, meanviolation, cost] = evaluation(problem, xfinal, condet);
        % maxviolation = max(maxviolation, manifoldViolation(xfinal));
 ...
 %   end
    ...
    if specifier.ind(7)
        ...
        time = toc(timetic);
        filename = sprintf('RC_nn_Riemannian_SQP_nrep%dRowdim%dColdim%dRank%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank);
        struct2csv(info, filename);        
        [maxviolation, meanviolation, cost] = evaluation(problem, xfinal, condet);
        % maxviolation = max(maxviolation, manifoldViolation(xfinal));
    ...
    filename = sprintf('RC_Info_nrep%dRowdim%dColdim%dRank%d.csv',setting.repeat,setting.row_dim, setting.col_dim, setting.rank);
