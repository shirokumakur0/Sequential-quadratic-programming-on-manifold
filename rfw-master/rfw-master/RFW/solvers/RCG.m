function varargout = RCG(fns,params)
% Solver of Riemannian conjugate gradient method
%
% INPUT:
...
%     params.dim [inf]          : the dimension of the manifold.
...
% By Wen Huang
    ...
    params = set_default_para(params, 'dim', inf, 'int', 1, inf);
    params = set_default_para(params, 'debug', 1, 'int', 0, 3);
    params = set_default_para(params, 'linesearch', 2, 'int', 1, 3);
    ...
    params = set_default_para(params, 'initstepsize', 0, 'float', 0, inf);
    ...
    while (err > params.error && times < params.max_t)
        ...
        if(params.linesearch == 1)
            [new_eta, t, x2, f2, gradf2, lnf, lng, lnV, lnR, ls, timeinfo] = linesearch_exact(fns, eta, x1, f1, gradf1, initslope, initstepsize, params, timeinfo);
        elseif(params.linesearch == 2)
            if(err_g < params.accuracy)
                [new_eta, t, x2, f2, gradf2, lnf, lng, lnV, lnR,ls, lnftime, lngtime, lnRtime, lnVtime] = choose_step_size(fns, eta, x1, f1, gradf1, params);
            else
                [new_eta, t, x2, f2, gradf2, lnf, lng, lnV, lnR, ls, timeinfo, newslope, lnftime, lngtime, lnRtime, lnVtime] = linesearch_Strong_Wolfe(fns, eta, x1, f1, gradf1, initslope, initstepsize, params, timeinfo);
            end
        end
        if(params.debug > 1)
            timeinfo = sprintf('time, linesearch total:%.2e,', toc - linesearchtimestart);
            getetatimestart = toc;
        end
        if(abs(t) < eps)
            fprintf('step size is zero, break!\n');
            break;
        end
        
        if(mod(times + 1, params.dim) ~= 0)
            Tvtimestart = toc;
            [Tgf1, new_eta, x1, x2] = fns.Tranv(x1, new_eta, x2, gradf1);
            Tv(times + 1) = Tv(times + 1) + toc - Tvtimestart;
            
            if(params.debug > 2)
                cprintf([1,0,1], sprintf('\t\tCheck isometry of vector transport: |gf| / |T gf|: %e/%e \n', ...
                    sqrt(fns.inpro(x1, gradf1, gradf1)), sqrt(fns.inpro(x2, Tgf1, Tgf1))));
            end
            % Polak-Ribiere
            sigma = fns.inpro(x2, gradf2, fns.TVaddTV(x2, gradf2, fns.STV(x2, -1, Tgf1))) / fns.inpro(x1, gradf1, gradf1);
            if(params.linesearch == 2)
                sigma = max(sigma, 0);
            end
            Tvtimestart = toc;
            [trveta, new_eta, x1, x2] = fns.Tranv(x1, new_eta, x2, eta);
            Tv(times + 1) = Tv(times + 1) + toc - Tvtimestart;
            if(fns.inpro(x2, trveta, gradf2) <= 0 || params.linesearch == 1) %% safeguard;
                eta = fns.TVaddTV(x2, fns.STV(x2, -1, gradf2), fns.STV(x2, sigma / fns.beta(x1, new_eta, x2), trveta));
            else
                eta = fns.STV(x2, -1, gradf2);
            end
            nV = nV + 2;
        else
            eta = fns.STV(x2, -1, gradf2);
        ...
        if(params.debug > 1)
            timeinfo = [timeinfo sprintf('get eta:%.2e,', toc - getetatimestart)];
        end
        ...
        if(params.debug > 0)
            fprintf('i:%d,f:%.3e,df/f:%.3e,|gf|:%.3e,initstepsize:%.3e,stepsize:%.3e,time:%.2e,stopls:%d\n', ...
                times, f2, err_r, err_g, initstepsize, t, T(end), ls);
        end
    end
    timecost = toc;
