    ...
    problem.ineq_constraint_hess{1} = @(x, u) [0;0];
    problem.ineq_constraint_cost{2} = @(x) I2' * x;
    problem.ineq_constraint_grad{2} = @(x) I2;
    problem.ineq_constraint_hess{2} = @(x, u) [0;0];
    ...
    problem.eq_constraint_hess{1} = @(x, u) [0;0];
    ...
    options.condet = constraintsdetail(problem);
    % Solve.
    %[x, xcost, info] = steepestdescent(problem);
    [x, xcost] = sqp(problem, problem.M.rand(), options) %#ok<ASGLU>
