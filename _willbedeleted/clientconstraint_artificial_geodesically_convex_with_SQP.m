function data = clientconstraint_artificial_geodesically_convex_with_SQP(m, methodoptions, specifier, setting)

data = NaN(3,6);
manifold = euclideanfactory(m);
problem.M = manifold;
problem.M.inner = @(x, d1, d2) ginner(x,d1,d2);
problem.M.norm = @(x, d1) gnorm(x,d1);
problem.M.dist = 'Not implimented';
problem.M.egrad2rgrad = @(x, g) e2rgrad;
problem.M.ehess2rhess = @(x, eg, eh, d) e2rhess;

    function val = ginner(x, d1, d2)
        val = 0;
        for i = 1:m
            val = val + (d1(i) * d2(i) / (x(i)^2));
        end
    end

    function val = gnorm(x, d1)
        val = ginner(x, d1, d1);
    end

    function val = e2rgrad(x, g)
       val = g;
       for i = 1:m
           val(i) = val(i) * x(i)^2;
       end
    ...
    function val = e2rhess(x, eg, eh, d)
       val = eh;
       for i = 1:m
          val(i) = val(i) -  
       end
...
ineq_constraints_cost = cell(m+1);
ineq_constraints_cost{1} = @(u) colones.' * u -1;

ineq_constraints_grad = cell(m+1);

ineq_constraints_hess = cell(m+1);
...
%     ------------------------- Solving ---------------------------
    ...
    if specifier.ind(4)
        ...
        filename = sprintf('BC_fmincon_interior_point_nrep%dDim%dDen%.3fTol%d.csv',setting.repeat,setting.dim, setting.density, setting.tolKKTres);
