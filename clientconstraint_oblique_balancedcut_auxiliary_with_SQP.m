function data = clientconstraint_oblique_balancedcut_auxiliary_with_SQP(L, rankY, methodoptions, specifier, setting)
...
%     ------------------------- Solving ---------------------------
    ...
    if specifier.ind(1)
        ...
        filename = sprintf('Auxiliary_BC_Mini-Sum-Max_nrep%dDim%dDen%.3f.csv',setting.repeat,setting.dim, setting.density);
    ...
    if specifier.ind(2)
        ...
        time = toc(timetic);
        filename = sprintf('Auxiliary_BC_ALM_nrep%dDim%dDen%.3f.csv',setting.repeat,setting.dim, setting.density);
    ...
    if specifier.ind(3)
        ...
        filename = sprintf('Auxiliary_BC_LQH_nrep%dDim%dDen%.3f.csv',setting.repeat,setting.dim, setting.density);
        struct2csv(info, filename);
    ...
    if specifier.ind(4)
        ...
        filename = sprintf('Auxiliary_BC_LSE_nrep%dDim%dDen%.3f.csv',setting.repeat,setting.dim, setting.density);
        struct2csv(info, filename);
    ...
    if specifier.ind(5)
        ...
        history.iter(1,:) =[];
        history.cost(1,:) = [];
        filename = sprintf('Auxiliary_BC_fmincon_interior_point_nrep%dDim%dDen%.3f.csv',setting.repeat,setting.dim, setting.density);
        struct2csv(info, filename);      
    ...
    if specifier.ind(6)
        ...
        history.iter(1,:) =[];
        history.cost(1,:) = [];
        filename = sprintf('Auxiliary_BC_fmincon_SQP_nrep%dDim%dDen%.3f.csv',setting.repeat,setting.dim, setting.density);
        struct2csv(info, filename);      
    ...
    if specifier.ind(7)
        % Riemannian SQP
        for sqp_rep = 1:setting.sqp_repeat
            ...
            time = toc(timetic);
            filename = sprintf('Auxiliary%d_BC_Riemannian_SQP_nrep%dDim%dDen%.3f.csv',sqp_rep,setting.repeat,setting.dim, setting.density);
            struct2csv(info, filename);
        ...
        filename = sprintf('Auxiliary_BC_Info_nrep%dDim%dDen%.3f.csv',setting.repeat,setting.dim, setting.density);
